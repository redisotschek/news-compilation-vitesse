...
const inputId = computed(() => {
  const id = props.id;
  if (!id) return `v-text-field-${Math.random().toString(36).substring(2, 15)}`;
  return id;
});
...
<template>
  <div
    class="text-left text-gray-500 border-0 rtl:text-right dark:text-gray-100 text-field--wrapper"
    :class="[variantClasses, block ? 'w-full' : 'w-max']"
  >
    <slot name="label">
      <label :for="inputId" class="text-left rtl:text-right">{{ label }}</label>
    </slot>
    <div class="relative flex items-center">
      <div
        v-if="slots.prepend || prepend"
        class="absolute flex items-center left-2 rtl:left-auto rtl:right-2"
      >
        <slot name="prepend">
      ...
      <div
        ...
        class="absolute flex items-center right-2 rtl:right-auto rtl:left-2"
      >
        <slot name="append">
