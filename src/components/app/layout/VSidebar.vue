          ...
          <nav class="flex-1 mt-5 text-xs font-medium leading-5 md:text-sm">
            <ul class="max-h-screen m-0 ml-0 list-none">
              <li v-for="(item, index) in sidebarItems" :key="item.title"
                ...
                sidebar-item py-4  px-4 flex items-center text-gray-600 dark:text-gray-300 rtl:space-x-reverse space-x-2 cursor-pointer  decoration-none  leading-5  group  focus:outline-none  transition duration-150 ease-in-out`"
                ...
                <CollapseTransition>
                  <ul v-if="item.children && !sidebarCollapsed && currentIndex === index"
                    ...
                    <li v-for="child in item.children" :key="child.title" class="pl-4 hover:scale-105 transition-all">
                      <RouterLink
                        :class="` py-2 text-xs  px-4 flex items-center rtl:space-x-reverse space-x-2  decoration-none  leading-5  group  focus:outline-none focus:font-bold transition duration-150 ease-in-out`"
